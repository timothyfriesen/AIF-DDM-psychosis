...
model = "RL_ddm" # RL, RL_ddm,RL_ddm_biased, AI, or AI_ddm
...
dataset = 'synthetic_dataset_RL-DDM' # Name of the dataset
...
file_paths = glob.glob(os.path.join(path_to_data, "*_game.csv"))
...
def process_participant(par):
    ...
    # Identify bad trials
    if dataset.startswith("synthetic_dataset") or dataset == "magic_carpet_2020":
        badtrials_rt1 = np.where((df_par['rt1'] < 0.1) | 
                             (df_par['rt1'] == -1) | 
                             df_par['rt1'].isna())[0]
    
        badtrials_rt2 = np.where((df_par['rt2'] < 0.1) | 
                             (df_par['rt2'] == -1) | 
                             df_par['rt2'].isna())[0]
